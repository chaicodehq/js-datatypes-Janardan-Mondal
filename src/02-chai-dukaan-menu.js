...
export function formatChaiMenu(items) {
  ...
  const validItems = items.filter(item => typeof item.price === "number" && item.price > 0 && typeof item.name === "string" && item.name.trim() !== "")

  let formattedArr = validItems.map(item => `${item.name.toUpperCase()} - Rs.${item.price}`)
